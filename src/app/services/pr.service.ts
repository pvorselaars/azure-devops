...
export class PrService {
  ...
  getPullRequests(status: 'active' | 'completed' = 'active' ): Observable<PullRequest[]> {
    const url = `_apis/git/pullrequests?searchCriteria.status=${status}&api-version=7.1`;
  ...
  getOpenPullRequests(): Observable<PullRequest[]> {
    return this.getPullRequests('active').pipe(
      map(prs => {
  ...
  getCompletedPullRequests(): Observable<PullRequest[]> {
    return this.getPullRequests('completed').pipe(
      map(prs => {
        return prs.slice().sort((a, b) => Date.parse(b.closedDate!) - Date.parse(a.closedDate!));
      }),
    );
  }
  ...
  private enrich(prs: PullRequest[]): Observable<PullRequest[]> {
    const perPrObservables = prs.map(pr =>
      forkJoin({
        ...
        environments: pr.lastMergeCommit ? this.deploymentService.getDeployedEnvironments(pr.lastMergeCommit.commitId).pipe(take(1), catchError(() => of([]))) : of([])
      })
    );
    ...
    return forkJoin(perPrObservables).pipe(
      map(results =>
        prs.map((pr, idx) => {
          const { policyStatuses = [], comments = [], environments = [] } = results[idx] ?? {};
          ...
          return { ...pr, approvals, policies, comments: commentCount, passRate, environments };
        })
      )
    );
  }
  ...
  private getPolicyStatus(projectId: string, pullRequestId: number): Observable<PolicyEvaluationRecord[]> {
    const url = `_apis/policy/evaluations?artifactId=vstfs:///CodeReview/CodeReviewId/${projectId}/${pullRequestId}&api-version=7.1-preview`;
  ...
  private getComments(pr: PullRequest): Observable<any[]> {
    const url = `_apis/git/repositories/${pr.repository.id}/pullRequests/${pr.pullRequestId}/threads?api-version=7.1`;
